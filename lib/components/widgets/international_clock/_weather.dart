...
class Weather {
  ...
  Future<bool> getData(String town) async {
    String logPrefix = "Weather | getData()";
    bool finalResponse = false;
    String url =
        "https://api.weatherapi.com/v1/forecast.json?key=$weatherKey%20&q=$town&days=1&aqi=no&alerts=no";
    log.debug(logPrefix, "URL: $url");
    try {
      var response = await http.get(Uri.parse(url));
      Map<String, dynamic> jsontemp = jsonDecode(response.body);
      this.time = jsontemp["current"]["last_updated"]
